...
router.post('/points', verifyToken, async (req, res) => {
    try {
        ...
        const point = await prisma.analysisPoint.create({
            ...
            include: {
                assessment: {
                    select: { id: true, name: true, entity: { select: { legalName: true } } }
                }
            }
        });
...
router.patch('/points/:id', verifyToken, async (req, res) => {
    try {
...
router.delete('/points/:id', verifyToken, async (req, res) => {
    try {
        const existing = await prisma.analysisPoint.findUnique({ where: { id: req.params.id } });
        if (!existing) return res.status(404).json({ error: 'Not found' });
