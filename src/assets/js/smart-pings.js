...
const SmartPings = (() => {
    ...
    async function scanAndGenerate() {
        ...
        // 5. Scan Strategic Tools completion
        scanStrategicProgress();

        // 6. Scan Contracts/Expiry (from localStorage diagnostic data)
        scanDiagnosticAlerts();
        ...
        data.dimensions.forEach(dim => {
            if (dim.score < 40) {
                _pings.push({
                    ...
                    severity: 'URGENT',
                    icon: '🏥',
                    title: `صحة "${s(dim.dimensionAr || dim.dimension)}" حرجة: ${Math.round(dim.score)}%`,
                ...
                _pings.push({
                    ...
                    severity: 'WARNING',
                    icon: '🩺',
                    title: `صحة "${s(dim.dimensionAr || dim.dimension)}" تحتاج تحسين: ${Math.round(dim.score)}%`,
    ...
    function scanStrategicProgress() {
        // Check if key tools are completed
        const analyses = JSON.parse(localStorage.getItem('stratix_completed_analyses') || '{}');
        const swotDone = analyses.SWOT || false;
        const pestelDone = analyses.PESTEL || false;
        const directionsDone = localStorage.getItem('stratix_identity_payload');

        if (!pestelDone && !swotDone) {
            _pings.push({
                id: 'strategy-start',
                severity: 'WARNING',
                icon: '🧭',
                title: 'لم تبدأ التحليل الاستراتيجي بعد',
                desc: 'ابدأ بـ PESTEL → Porter → SWOT لفهم بيئتك',
                actions: [
                    { label: '🌍 ابدأ PESTEL', href: '/tool-detail.html?code=PESTEL' }
                ]
            });
        } else if (swotDone && !directionsDone) {
            _pings.push({
                id: 'strategy-directions',
                severity: 'INFO',
                icon: '🎯',
                title: 'التحليل مكتمل — حان وقت تحديد الرؤية',
                desc: 'أكملت SWOT — حدد الرؤية والرسالة والقيم',
                actions: [
                    { label: '🎯 الهوية المؤسسية', href: '/directions.html' }
                ]
            });
        }
    }
    ...
    // ── 6. Diagnostic Alerts ──
    function scanDiagnosticAlerts() {
        const diag = JSON.parse(localStorage.getItem('stratix_diagnostic_payload') || 'null');
        if (!diag) return;

        // Check compliance score
        if (diag.complianceScore !== undefined && diag.complianceScore < 40) {
            _pings.push({
                id: 'diag-compliance-critical',
                severity: 'URGENT',
                icon: '⚖️',
                title: `درجة الامتثال حرجة: ${diag.complianceScore}%`,
                desc: 'خطر مخالفات وغرامات — راجع وضعك التنظيمي فوراً',
                actions: [
                    { label: '⚖️ الامتثال', href: '/dept-deep.html?dept=compliance' }
                ]
            });
        }

        // Check maturity level
        if (diag.maturityLevel === 'nascent' || diag.overallScore < 30) {
            _pings.push({
                id: 'diag-maturity-low',
                severity: 'WARNING',
                icon: '🌱',
                title: 'مستوى النضج: ناشئ',
                desc: 'شركتك في بداية رحلة النضج — ركّز على الأساسيات',
                actions: [
                    { label: '📊 التشخيص', href: '/diagnostic-center.html' }
                ]
            });
        }
    }
