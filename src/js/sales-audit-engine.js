/**
 * Startix — Sales Audit Pro Engine
 * منطق التقييم + الحفظ التلقائي + عرض المحاور
 */
window.SalesAuditEngine = (function () {
    const CFG = window.SalesAuditConfig;
    const STORAGE_KEY = 'startix_sales_audit';
    ...
    function renderMetaForm() {
        ...
        container.innerHTML = `
            <a href="/dept-dashboard.html" class="back-link">
                <i class="bi bi-arrow-right"></i>
                العودة للوحة الإدارات
            </a>
    ...
    async function loadSectors() {
        ...
        try {
            ...
            // Fallback sectors
            ['saas:SaaS', 'retail:تجزئة', 'services:خدمات', 'industrial:صناعي', 'fnb:مطاعم', 'real_estate:عقارات', 'health:صحة', 'education:تعليم'].forEach(s => {
                const [k, l] = s.split(':');
                const opt = document.createElement('option');
                opt.value = k;
                opt.textContent = l;
    ...
    function bindMetaEvents() {
        ...
        Object.entries(fields).forEach(([elId, key]) => {
            ...
            // Also listen change for selects
    ...
    function renderAxesNav() {
        ...
        nav.innerHTML = axes.map(a => {
            ...
            return `
                <div class="axis-nav-item ${active} ${empty}" data-axis="${a.id}" onclick="SalesAuditEngine.goToAxis(${a.id})">
    ...
    function renderCurrentAxis() {
        ...
        html += `
            <div class="axis-nav-buttons">
                ${currentIdx > 0 ? `<button class="btn-audit-secondary" onclick="SalesAuditEngine.goToAxis(${axes[currentIdx - 1].id})"><i class="bi bi-arrow-right"></i> المحور السابق</button>` : '<div></div>'}
                ${currentIdx < axes.length - 1 ? `<button class="btn-audit-primary" onclick="SalesAuditEngine.goToAxis(${axes[currentIdx + 1].id})">المحور التالي <i class="bi bi-arrow-left"></i></button>` : `<button class="btn-audit-primary" onclick="SalesAuditEngine.showSummary()"><i class="bi bi-file-earmark-bar-graph"></i> عرض الملخص</button>`}
            </div>
        `;
    ...
    function renderItem(item, color) {
        ...
        return `
            <div class="audit-item" data-item="${item.id}">
                ...
                <div class="item-note-toggle" onclick="SalesAuditEngine.toggleNote('${item.id}')">
        ...
        container.innerHTML = `
            <div class="audit-card summary-card">
                ...
                <div class="summary-actions">
                    <button class="btn-audit-secondary" onclick="SalesAuditEngine.goToAxis(1)">
