...
window.DeptSmartEngine = (function () {
    ...
    // ── RENDER KPIs (مع Prefill تلقائي من التحليل السابق) ──────────────
    function renderKpis() {
        // جلب البيانات السابقة من كل المصادر
        const prevData = _loadPreviousData();
        const alreadySaved = !!localStorage.getItem(`stratix_smart_${dept}`);

        // خريطة التحويل: حقول dept-deep → IDs KPI في dept-smart
        const _DEEP_TO_SMART = {
            // مبيعات — فقط الحقول بنفس الوحدة والمفهوم
            // annual_sales (ريال) ≠ k_kpi1 (% تحقيق هدف) — لا يُربط
            // win_rate (% فوز صفقة) ≠ k_kpi2 (% تحويل leads) — مفهوم مختلف
            sales_cycle: 'k_kpi3', // أيام = أيام ✅
            // active_customers (عدد) ≠ k_kpi4 (% احتفاظ) — وحدة مختلفة
            // HR
            new_hires: 'k_kpi1', turnover_rate: 'k_kpi2',
            enps: 'k_kpi3', vacancy_days: 'k_kpi4',
            // Finance
            net_profit_pct: 'k_kpi1', on_time_close: 'k_kpi2',
            budget_deviation: 'k_kpi3', dso: 'k_kpi4',
            // Operations
            efficiency: 'k_kpi1', on_time_delivery: 'k_kpi2',
            defect_rate: 'k_kpi3', downtime: 'k_kpi4',
            // IT
            uptime: 'k_kpi1', avg_resolution: 'k_kpi2',
            security_incidents: 'k_kpi3', infra_utilization: 'k_kpi4',
            // حجم الفريق والميزانية
            team_size: 'k_team_size', teamSize: 'k_team_size',
            budget: 'k_budget', annual_budget: 'k_budget'
        };

        // بناء كائن القيم الجاهزة
        const prefilled = {};
        if (!alreadySaved && prevData) {
            const srcs = [prevData, prevData.kpis, prevData.numbers, prevData.data, prevData.answers, prevData.fields];
            srcs.forEach(src => {
                if (!src || typeof src !== 'object') return;
                Object.entries(src).forEach(([k, v]) => {
                    const id = _DEEP_TO_SMART[k];
                    if (id && v != null && v !== '') prefilled[id] = v;
                });
            });
        }

        // عدد الحقول المستوردة
        const importedCount = Object.keys(prefilled).length;

        // banner الإشعار (يظهر فقط إذا وجدت بيانات سابقة)
        let banner = '';
        if (importedCount > 0) {
            banner = `
            <div style="
            ...
            ">
                <i class="bi bi-lightning-charge-fill" style="color:#10b981;font-size:16px;flex-shrink:0"></i>
                <div>
                    <span style="color:#10b981;font-weight:800">تم استيراد ${importedCount} حقل</span>
                    <span style="color:#94a3b8"> من تحليلك السابق تلقائياً — راجع الأرقام وعدّل إذا تغيّرت</span>
                </div>
        ...
        document.getElementById('kpiGrid').innerHTML = banner + deptKpis.map(k => {
            const importedVal = prefilled[k.id];
            const hasImport = importedVal != null;

            return `
            <div class="kpi-card${k.isbudget ? ' budget' : ''}" style="${hasImport ? 'border-color:rgba(16,185,129,0.25)' : ''}">
                <div class="kpi-label">
                    <i class="bi ${k.icon}"></i> ${k.label}
                    ${hasImport ? `<span style="
                        font-size:9px;font-weight:800;padding:2px 7px;border-radius:4px;
                        background:rgba(16,185,129,0.12);color:#10b981;margin-right:6px;
                    ">✓ من تحليلك السابق</span>` : ''}
                </div>
                <input class="kpi-input" type="number" id="${k.id}"
                    value="${hasImport ? importedVal : ''}"
                    placeholder="${k.placeholder}" min="0" ${k.max ? `max="${k.max}"` : ''}
                    oninput="onKpiChange()"
                    style="${hasImport ? 'border-color:rgba(16,185,129,0.3);background:rgba(16,185,129,0.04)' : ''}">
                <div class="kpi-hint">${k.hint}</div>
            </div>`;
        }).join('');

        // تسجيل القيم المستوردة في الـ state
        Object.entries(prefilled).forEach(([id, val]) => {
            state.kpis[id] = +val;
        });
    }

    // ── جلب البيانات من المراحل السابقة (dept-deep + diagnostic) ──────────
    function _loadPreviousData() {
        // لا تستورد إذا يوجد بيانات smart محفوظة (تعديلات يدوية)
        if (localStorage.getItem(`stratix_smart_${dept}`)) return null;

        // DataBridge أولاً
        if (window.DataBridge) {
            return window.DataBridge.getDeptDeep(dept) || window.DataBridge.getDiagnostic();
        }

        // Fallback يدوي
        let d = null;
        try { d = JSON.parse(localStorage.getItem(`DEEP_${dept.toUpperCase()}`) || 'null'); } catch (e) { }
        if (!d) {
            try {
                const p = JSON.parse(localStorage.getItem('stratix_dept_deep_payload') || '{}');
                d = p[dept] || null;
            } catch (e) { }
        }
        return d;
    ...
    async function saveReport() {
        ...
        try {
            if (window.DeptPage) {
                ...
                await window.api.post('/api/dept/analysis', { dept, type: 'SMART', data: payload });
                apiSaved = true;
            }
        } catch (e) {
    ...
    // ── IMPORT FROM PREVIOUS STAGES (عبر DataBridge الموحد) ────────────
    function importFromDeepAnalysis() {
        // لا تستورد إذا عنده بيانات smart محفوظة مسبقاً
        const alreadySaved = localStorage.getItem(`stratix_smart_${dept}`);
        ...
        // ── المسار الأول: DataBridge (إذا محمّل) ──
        if (window.DataBridge) {
            const prefill = window.DataBridge.getPrefillData(dept, 'dept-smart');
            kpis = prefill.kpis || {};
            axes = prefill.axes || {};
            console.log(`[DeptSmart] 📥 DataBridge prefill for [${dept}]:`, prefill);
        } else {
            // ── المسار الثاني: fallback يدوي (نفس المنطق السابق) ──
            let deepData = null;
            try { deepData = JSON.parse(localStorage.getItem(`DEEP_${dept.toUpperCase()}`) || 'null'); } catch (e) { }
            if (!deepData) {
                try {
                    const p = JSON.parse(localStorage.getItem('stratix_dept_deep_payload') || '{}');
                    deepData = p[dept] || null;
                } catch (e) { }
            }
            if (!deepData) {
                try { deepData = JSON.parse(localStorage.getItem(`DEEP_ANALYSIS_${dept.toUpperCase()}`) || 'null'); } catch (e) { }
            }

            if (deepData) {
                // خريطة تحويل حقول dept-deep → IDs في dept-smart
                const kpiMapping = {
                    totalRevenue: 'k_revenue', monthly_revenue: 'k_revenue',
                    conversionRate: 'k_conversion', conversion_rate: 'k_conversion',
                    teamSize: 'k_team_size', team_size: 'k_team_size',
                    newCustomers: 'k_customers',
                    headcount: 'k_headcount', total_employees: 'k_headcount',
                    turnoverRate: 'k_turnover', turnover_rate: 'k_turnover',
                    netProfit: 'k_profit', cashFlow: 'k_cashflow'
                };
                [deepData.kpis, deepData.numbers, deepData.data, deepData.answers, deepData.metrics, deepData]
                    .forEach(src => {
                        if (!src || typeof src !== 'object') return;
                        Object.entries(src).forEach(([k, v]) => {
                            const id = kpiMapping[k];
                            if (id && v != null && v !== '') kpis[id] = v;
                        });
                    });
                axes = deepData.axes || deepData.sections || deepData.pillars || {};
            }
        }
        ...
        // ── حقن المحاور في الـ UI ──
        Object.entries(axes).forEach(([axId, axData]) => {
            let score = typeof axData === 'number'
                ? Math.round(axData)
                : Math.round(+(axData?.score ?? axData?.maturity ?? axData?.value ?? 0));
            if (!score) return;
            if (score > 4) score = Math.max(1, Math.min(4, Math.round(score / 25)));
            if (score < 1) score = 1;

            state.axes[axId] = score;
            document.querySelectorAll(`[data-axis="${axId}"]`).forEach(o => {
                if (+o.dataset.score === score) {
        ...
        if (imported > 0) {
            ...
            if (toast && toastMsg) {
                toastMsg.textContent = '✅ تم استيراد البيانات من مراحل التحليل السابقة';
                toast.classList.add('show');
                setTimeout(() => toast.classList.remove('show'), 4000);
            }
            console.log(`[DeptSmart] ✅ Imported ${imported} fields from previous stages`);
        }
    }

    // ── RESTORE SAVED DATA ────────────────────────────────
    async function restoreState() {
        // 1. Try loading from API first (DB is source of truth)
        let d = null;
        try {
            if (window.api?.get) {
                const res = await window.api.get(`/api/dept/analysis?dept=${dept}&type=SMART`);
                if (res?.success && res.data) {
                    d = typeof res.data === 'string' ? JSON.parse(res.data) : res.data;
                    // Sync to localStorage as cache
                    localStorage.setItem(`stratix_smart_${dept}`, JSON.stringify(d));
                }
            }
        } catch (e) {
            console.warn('[SmartEngine] API load failed, falling back to localStorage:', e.message);
        }

        // 2. Fallback to localStorage
        if (!d) {
            let saved = localStorage.getItem(`stratix_smart_${dept}`);
            d = saved ? JSON.parse(saved) : { kpis: {}, axes: {}, notes: {}, problems: {}, resources: {} };
        }

        // 🔗 [DataBridge Connection] — منع التكرار (Redundancy Fix)
        if (window.DataBridge) {
            const prefill = DataBridge.getPrefillData(dept, 'dept-smart');
            if (prefill && prefill.kpis) {
                console.log(`[SmartEngine] 🔗 Syncing from DataBridge for ${dept}`);
                Object.entries(prefill.kpis).forEach(([k, v]) => {
                    // فقط إذا كانت القيمة فارغة في التخزين الحالي
                    if (v != null && (d.kpis[k] === undefined || d.kpis[k] === '' || d.kpis[k] === 0)) {
                        d.kpis[k] = v;
                    }
                });
            }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        renderAxes();
        renderProblems();
        restoreState();          // أولاً: استعادة بيانات smart المحفوظة مسبقاً
        importFromDeepAnalysis(); // ثانياً: استيراد من dept-deep إذا لا يوجد smart saved
        updateProgress();
    });
    ...
    return { renderKpis, renderAxes, renderProblems, restoreState, importFromDeepAnalysis, updateProgress, updateReport, loadAndRenderFines };
})();
