...
(function () {
  ...
  function getLocalSuggestions(page) {
    const tips = [];
    let toolCode = page.includes('code=') ? page.split('code=')[1].split('&')[0] : '';
    if (!toolCode) {
      if (page.includes('bcg-matrix')) toolCode = 'BCG_MATRIX';
      else if (page.includes('ansoff-matrix')) toolCode = 'ANSOFF_MATRIX';
      else if (page.includes('swot')) toolCode = 'SWOT';
      else if (page.includes('tows')) toolCode = 'TOWS';
    }

    // === قراءة قطاع الشركة لتخصيص الاقتراحات ===
    let sector = '';
    try {
      const raw = localStorage.getItem('onboardingData') || localStorage.getItem('companyData') || '{}';
      const data = JSON.parse(raw);
      sector = (data.sector || data.industry || localStorage.getItem('companySector') || '').toLowerCase();
    } catch (e) { }
    ...
    // === تخصيص حسب القطاع ===
    if (sector.includes('تقن') || sector.includes('tech') || sector.includes('برمجيات')) {
      pestelSections = {
        political: { label: '🏛️ سياسي', items: ['تشريعات الذكاء الاصطناعي والبيانات', 'دعم حكومي للشركات التقنية', 'سياسات توطين البيانات السحابية', 'قوانين تنظيم المحتوى الرقمي'] },
        economic: { label: '💰 اقتصادي', items: ['توفر الاستثمارات الجريئة (VCs)', 'تكلفة البنية التحتية السحابية', 'تأثير التضخم على ميزانيات IT للشركات', 'أسعار الرخص والاشتراكات بالدولار'] },
        social: { label: '👥 اجتماعي', items: ['نقص الكفاءات التقنية المتخصصة', 'تبني المجتمع للحلول الرقمية', 'تفضيل العمل عن بُعد', 'مخاوف الخصوصية لدى المستخدمين'] },
        technological: { label: '💻 تقني', items: ['تطور الذكاء الاصطناعي التوليدي (GenAI)', 'تحديثات الأمن السيبراني', 'تقنيات الحوسبة الطرفية (Edge)', 'تطور واجهات البرمجة (APIs)'] },
        environmental: { label: '🌿 بيئي', items: ['استهلاك الطاقة لمراكز البيانات', 'التخلص من النفايات الإلكترونية (e-waste)', 'مبادرات التقنية الخضراء', 'البصمة الكربونية للعمليات الرقمية'] },
        legal: { label: '⚖️ قانوني', items: ['نظام حماية البيانات الشخصية (PDPL)', 'حقوق الملكية الفكرية للبرمجيات', 'قوانين الجرائم المعلوماتية', 'تراخيص هيئة الاتصالات (CST)'] }
      };
    } else if (sector.includes('طب') || sector.includes('صح') || sector.includes('health') || sector.includes('medical') || sector.includes('عياد')) {
      pestelSections = {
        political: { label: '🏛️ سياسي', items: ['ميزانية الإنفاق الحكومي على الصحة', 'سياسات خصخصة القطاع الصحي', 'تشريعات وزارة الصحة الجديدة', 'سياسات التأمين الصحي الإلزامي'] },
        economic: { label: '💰 اقتصادي', items: ['تكلفة المعدات الطبية والأدوية', 'التأمين الصحي وأسعار التغطية', 'القدرة الشرائية للخدمات التجميلية/الاختيارية', 'تمويل المشاريع الصحية'] },
        social: { label: '👥 اجتماعي', items: ['زيادة الوعي الصحي والوقائي', 'نمو نسبة كبار السن بالمجتمع', 'تغير نمط الحياة والأمراض المزمنة', 'الإقبال على الطب البديل/التكميلي'] },
        technological: { label: '💻 تقني', items: ['تطور التطبيب عن بُعد (Telehealth)', 'السجلات الطبية الإلكترونية (الملف الموحد)', 'الذكاء الاصطناعي في التشخيص', 'تقنيات الجراحة الروبوتية'] },
        environmental: { label: '🌿 بيئي', items: ['التخلص الآمن من النفايات الطبية', 'معايير مكافحة العدوى والتعقيم', 'تأثير التلوث على الصحة العامة', 'كفاءة استهلاك الطاقة في المستشفيات'] },
        legal: { label: '⚖️ قانوني', items: ['قوانين الأخطاء الطبية', 'تراخيص مزاولة المهن الصحية', 'تسجيل الأدوية في الغذاء والدواء (SFDA)', 'سرية بيانات المرضى'] }
      };
    } else if (sector.includes('تجزئة') || sector.includes('بيع') || sector.includes('retail') || sector.includes('متاجر')) {
      pestelSections = {
        political: { label: '🏛️ سياسي', items: ['سياسات التوطين في منافذ البيع', 'قوانين الاستيراد والجمارك', 'الرسوم البلدية على اللوحات والمتاجر', 'دعم المحتوى المحلي'] },
        economic: { label: '💰 اقتصادي', items: ['تأثير التضخم على القوة الشرائية', 'تكاليف الإيجارات التجارية', 'أسعار الشحن وسلاسل الإمداد', 'الضريبة الانتقائية وضريبة القيمة المضافة'] },
        social: { label: '👥 اجتماعي', items: ['تغير تفضيلات المستهلكين', 'التحول للتسوق الإلكتروني', 'تأثير المؤثرين (Influencers) على الشراء', 'مواسم الأعياد والتخفيضات'] },
        technological: { label: '💻 تقني', items: ['تطور منصات التجارة الإلكترونية', 'أنظمة الدفع الرقمي (Apple Pay/STC Pay)', 'أتمتة المخازن ونقاط البيع (POS)', 'تحليل بيانات العملاء وبرامج الولاء'] },
        environmental: { label: '🌿 بيئي', items: ['التوجه نحو التغليف الصديق للبيئة', 'الحد من البلاستيك أحادي الاستخدام', 'استدامة سلاسل التوريد', 'إدارة النفايات في المتاجر'] },
        legal: { label: '⚖️ قانوني', items: ['قوانين حماية المستهلك', 'شروط وزارة التجارة للتخفيضات', 'سياسات الاسترجاع والاستبدال', 'متطلبات رخص البلدية والدفاع المدني'] }
      };
    }
    ...
    const TOOL_SUGGESTIONS = {
      PESTEL: {
        ...
        message: 'اختر العناصر المناسبة لقطاعك — اضغط على أي عنصر لإضافته مباشرة:',
        sections: pestelSections
      },
      PORTER: {
      ...
      CORE_COMPETENCY: {
        icon: '🏆', title: '🤖 اقتراحات للقدرات الجوهرية',
        message: 'حدد ما يميز شركتك ولا يمكن للمنافسين تقليده بسهولة:',
        sections: {
          items: { label: '🏆 قدرات جوهرية', items: ['براءة اختراع أو تقنية حصرية', 'ثقافة مؤسسية وبيئة عمل استثنائية', 'شبكة علاقات قوية وحصرية مع الموردين', 'قاعدة بيانات عملاء ضخمة وتحليلاتها', 'سرعة ابتكار المنتجات وإطلاقها', 'خبرة متراكمة في القطاع يصعب نقلها'] }
        }
      },
      CUSTOMER_JOURNEY: {
        icon: '🚶', title: '🤖 اقتراحات لرحلة العميل',
        message: 'حدد أهم نقاط التواصل في تجربة عميلك:',
        sections: {
          items: { label: '📍 نقاط تواصل', items: ['إعلانات السوشيال ميديا (الوعي)', 'زيارة الموقع الإلكتروني', 'مكالمة مع فريق المبيعات', 'تجربة الاستخدام الأولى للمنتج', 'خدمة ما بعد البيع والدعم', 'برامج الولاء والمكافآت'] }
        }
      },
      EISENHOWER: {
        icon: '🔥', title: '🤖 اقتراحات لمصفوفة أيزنهاور',
        message: 'صنّف مبادراتك ومهامك حسب الأهمية والإلحاح:',
        sections: {
          doFirst: { label: '🔥 هام وعاجل (أنجز)', items: ['أزمة حالية مع عميل رئيسي', 'مشكلة تقنية توقف سير العمل', 'موعد تسليم مشروع نهائي', 'فرصة بيعية بوقت محدود'] },
          schedule: { label: '📅 هام وغير عاجل (خطّط)', items: ['التخطيط الاستراتيجي للربع القادم', 'تطوير مهارات الفريق', 'تحسين البنية التحتية التقنية', 'بناء شراكات جديدة'] },
          delegate: { label: '👥 غير هام وعاجل (فوّض)', items: ['الرد على الاستفسارات الروتينية', 'جدولة منشورات السوشيال ميديا', 'إعداد التقارير الدورية', 'المهام الإدارية المكررة'] },
          eliminate: { label: '❌ غير هام وغير عاجل (احذف)', items: ['الاجتماعات غير المنتجة', 'العمليات الورقية المعقدة', 'ميزات منتج لا تُستخدم', 'المهام التي لا تضيف قيمة'] }
        }
      },
      BCG_MATRIX: {
        icon: '⭐', title: '🤖 اقتراحات لمصفوفة BCG',
        message: 'صنّف منتجاتك ووحداتك حسب حصتها السوقية ومعدل نموها:',
        sections: {
          stars: { label: '⭐ نجوم (نمو عالي، حصة عالية)', items: ['المنتج الرقمي الأحدث للشركة', 'خدمة سحابية سريعة الانتشار', 'خط إنتاج مبتكر يتصدر السوق'] },
          cashCows: { label: '💰 أبقار نقدية (نمو منخفض، حصة عالية)', items: ['المنتج الأساسي الكلاسيكي', 'عقود الصيانة والدعم السنوية', 'الخدمات ذات الإيراد الثابت'] },
          questionMarks: { label: '❓ علامات استفهام (نمو عالي، حصة منخفضة)', items: ['تطبيق جديد في سوق مزدحم', 'خدمة تجريبية تحتاج تسويق مكثف', 'فرع جديد في منطقة نامية'] },
          dogs: { label: '🐕 كلاب (نمو منخفض، حصة منخفضة)', items: ['إصدار قديم من المنتج', 'خدمة تراجعت مبيعاتها مؤخراً', 'وحدة أعمال غير مربحة تستنزف الموارد'] }
        }
      },
      ANSOFF_MATRIX: {
        icon: '📈', title: '🤖 اقتراحات لمصفوفة أنسوف',
        message: 'حدد مسارات النمو لمنتجاتك وأسواقك:',
        sections: {
          penetration: { label: '🎯 اختراق السوق (حالي × حالي)', items: ['زيادة الحملات الترويجية والخصومات', 'تحسين برنامج ولاء العملاء', 'حملة لاستعادة العملاء السابقين', 'زيادة عمولة فريق المبيعات'] },
          productDev: { label: '📦 تطوير المنتج (جديد × حالي)', items: ['إطلاق ميزة جديدة للمنتج الحالي', 'تقديم نسخة "برو" باشتراك مميز', 'تطوير تطبيق جوال لخدمة موجودة'] },
          marketDev: { label: '🌍 تطوير السوق (حالي × جديد)', items: ['التوسع للبيع في دول الخليج', 'استهداف قطاع الشركات B2B بدلاً من B2C', 'فتح قنوات بيع إلكترونية جديدة'] },
          diversification: { label: '🚀 التنويع (جديد × جديد)', items: ['الاستحواذ على شركة تقنية ناشئة', 'دخول قطاع التقنية المالية (Fintech)', 'إطلاق خط إنتاج في قطاع مختلف كلياً'] }
        }
      }
    };
  ...
  function applySuggestion(sectionKey, text, chipEl) {
    ...
    // بحث احتياطي عام في حالة الصفحات الموحدة (مثل tool-detail.html)
    if (!input) {
      input = document.querySelector('input[placeholder*="إضافة"], input[placeholder*="أضف"], input.add-item-input');
    }
    ...
    // بحث احتياطي عن زر الإضافة في كامل الصفحة
    if (!addBtn) {
      addBtn = document.querySelector('button[onclick*="addItem"], button.btn-add');
    }
